...
import { ShareLinkApiService } from '@client/api';
...
export class ShareLinkService {
  ...
  createShareLink = (successCallback: () => void) =>
    injectMutation(() => ({
      mutationFn: (data: any) => this.shareLinkApiService.createShareLink(data),
      onSuccess: () => {
  ...
  deleteShareLink = () =>
    injectMutation(() => ({
      mutationFn: (data: any) => this.shareLinkApiService.deleteShareLink(data),
      onSuccess: () => {
        this.queryClient.invalidateQueries({ queryKey: ['shareLinks'] });
        this.notificationService.success(
