...
import { futureDateValidator } from '@client/shared/utils/validators';
...
export class DialogCreateShareLinkComponent {
  ...
  createShareLinkForm = new FormGroup({
    ...
    expiresAt: new FormControl<Date | null>(null, [futureDateValidator]),
  });
  ...
  togglePasswordField = () =>
    this.passwordFieldShown.update((prev) => {
      const newValue = !prev;
      !newValue &&
        this.createShareLinkForm.patchValue({
          password: '',
        });
      return newValue;
    });

  togglePasswordInput = () => this.passwordInputShown.update((prev) => !prev);

  toggleExpirationDate = () => this.expirationDateShown.update((prev) => !prev);
