...
import { ShareLinkService } from '@client/features/collection/data-access';
...
import { TitleTextActionComponent } from '@client/shared/components/title-text-action/title-text-action.component';
import {
  ButtonPrimaryDirective,
  StylingInputDirective,
} from '@client/shared/directives';
import { formatDate } from '@client/shared/utils/dayjs';
...
@Component({
  ...
  providers: [provideIcons({ phosphorTrash, phosphorCopy })],
...
export class ShareLinksComponent {
  ...
  checkActive = (linkObj: any) => {
    ...
    return (
      linkObj?.expiresAt &&
      (linkObj.expiresAt === null || expirationDate > currentDate)
    );
  };
  ...
  deleteShareLink = (id: string) =>
    this.dialog.open(DialogConfirmComponent, {
      data: {
        ...
        description:
          'Are you sure you want to delete this shareable link? This action cannot be undone.',
